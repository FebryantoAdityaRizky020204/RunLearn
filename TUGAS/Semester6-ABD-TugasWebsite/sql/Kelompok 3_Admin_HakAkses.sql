-- ==========================================
-- File Script SQL Kelompok 3
-- ==========================================

-- ==========================================
-- 1. MEMBUAT ROLE UNTUK SETIAP JENIS PENGGUNA
-- ==========================================
...
-- ==========================================
-- 2. MEMBERIKAN HAK AKSES DAN REVOKE
-- ==========================================
# ADMIN ROLE (Semua Hak Akses di db sahabat_satwa)
GRANT ALL PRIVILEGES ON sahabat_satwa.* TO 'admin_role';
...
-- Buat view untuk vet_self
CREATE VIEW vet_self AS 
SELECT * FROM vet WHERE vet_phone = SUBSTRING_INDEX(USER(), '@', 1); -- Asumsi vet_phone unik dan sesuai dengan username
GRANT SELECT ON sahabat_satwa.vet_self TO 'vet_role';

-- Jangan cabut hak akses SELECT ke tabel vet karena belum diberikan sebelumnya
-- REVOKE SELECT ON sahabat_satwa.vet FROM 'vet_role'; -- Hapus baris ini

# PET OWNER
-- Buat view untuk pet_owner_owners
CREATE VIEW pet_owner_owners AS 
SELECT * FROM owners WHERE owner_phone = SUBSTRING_INDEX(USER(), '@', 1); -- Asumsi owner_phone unik dan sesuai dengan username

-- Buat view untuk pet_owner_animal
CREATE VIEW pet_owner_animal AS 
SELECT a.* 
FROM animal a
JOIN owners o ON a.owner_id = o.owner_id
WHERE o.owner_phone = SUBSTRING_INDEX(USER(), '@', 1);

-- Buat view untuk pet_owner_visit
CREATE VIEW pet_owner_visit AS 
SELECT v.* 
FROM visit v
...
-- Berikan hak akses SELECT dan UPDATE pada view ke pet_owner_role
GRANT SELECT, UPDATE ON sahabat_satwa.pet_owner_owners TO 'pet_owner_role';
GRANT SELECT, UPDATE ON sahabat_satwa.pet_owner_animal TO 'pet_owner_role';
GRANT SELECT ON sahabat_satwa.pet_owner_visit TO 'pet_owner_role';

-- Cabut hak akses SELECT dan UPDATE langsung ke tabel owners, animal, dan visit
-- REVOKE SELECT, UPDATE ON sahabat_satwa.owners FROM 'pet_owner_role';
-- REVOKE SELECT, UPDATE ON sahabat_satwa.animal FROM 'pet_owner_role';
-- REVOKE SELECT ON sahabat_satwa.visit FROM 'pet_owner_role';

-- ==========================================
-- 3. MEMBUAT USER DAN MEMBERIKAN ROLE
-- ==========================================
CREATE USER 'admin_user' IDENTIFIED BY 'admin_password';
CREATE USER 'vet_user' IDENTIFIED BY 'vet_password';
CREATE USER 'pet_owner_user' IDENTIFIED BY 'owner_password';

GRANT 'admin_role' TO 'admin_user';
GRANT 'vet_role' TO 'vet_user';
GRANT 'pet_owner_role' TO 'pet_owner_user';


-- ==========================================
-- 3. CONTOH IMPLEMENTASI REVOKE HAK AKSES
-- ==========================================

# ADMIN ROLE: Menghapus hak DELETE dan DROP di seluruh database
REVOKE DELETE ON sahabat_satwa.* FROM 'admin_role';
REVOKE DROP ON sahabat_satwa.* FROM 'admin_role';

# VET ROLE: Menghapus hak INSERT pada visit_drug dan UPDATE pada visit
REVOKE INSERT ON sahabat_satwa.visit_drug FROM 'vet_role';
REVOKE UPDATE ON sahabat_satwa.visit FROM 'vet_role';

# PET OWNER ROLE: Menghapus hak UPDATE pada pet_owner_animal dan SELECT pada pet_owner_visit
REVOKE UPDATE ON pet_owner_animal FROM 'pet_owner_role';
REVOKE SELECT ON pet_owner_visit FROM 'pet_owner_role';

-- ==========================================
-- 4. IMPLEMENTASI REVOKE ROLE DAN DROP ROLE
-- ==========================================

-- Revoke semua hak akses sebelum menghapus role
# ADMIN ROLE
REVOKE ALL PRIVILEGES ON sahabat_satwa.* FROM 'admin_role';

# VET ROLE
REVOKE SELECT ON sahabat_satwa.owners FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.clinic FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.animal_type FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.specialisation FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.drug FROM 'vet_role';
REVOKE SELECT, UPDATE ON sahabat_satwa.animal FROM 'vet_role';
REVOKE SELECT, INSERT, UPDATE ON sahabat_satwa.visit FROM 'vet_role';
REVOKE SELECT, INSERT ON sahabat_satwa.visit_drug FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.spec_visit FROM 'vet_role';
REVOKE SELECT ON sahabat_satwa.vet_self FROM 'vet_role';

# PET OWNER ROLE
REVOKE SELECT, UPDATE ON sahabat_satwa.pet_owner_owners FROM 'pet_owner_role';
REVOKE SELECT, UPDATE ON sahabat_satwa.pet_owner_animal FROM 'pet_owner_role';

-- Menghapus peran dari masing-masing pengguna
REVOKE 'admin_role' FROM 'admin_user';
REVOKE 'vet_role' FROM 'vet_user';
REVOKE 'pet_owner_role' FROM 'pet_owner_user';

-- Menghapus role dari database
DROP ROLE 'admin_role';
DROP ROLE 'vet_role';
DROP ROLE 'pet_owner_role';

-- Menghapus user dari database
DROP USER 'admin_user';
DROP USER 'vet_user';
DROP USER 'pet_owner_user';

-- ==========================================
-- 5. IMPLEMENTASI ENKRIPSI DATA SENSITIF (Nomor Telepon)
-- Note: pada schema sebelumnya, tipe data owner_phone harus dirubah menjadi varbinary(255)
-- -> agar bisa menyimpan hasil enkripsi
-- ==========================================
INSERT INTO owners (owner_givenname, owner_familyname, owner_address, owner_phone)
VALUES ('John', 'Doe', '123 Main St', AES_ENCRYPT('081234567890', 'adit')); # nomor telepon dienkripsi menggunakan AES_ENCRYPT

# SELECT
SELECT owner_id, owner_givenname, owner_familyname, owner_address, CAST(AES_DECRYPT(owner_phone, 'adit') AS CHAR) AS owner_phone_decrypted
FROM owners;

-- ==========================================
-- 6. IMPLEMENTASI AUDIT LOG
-- ==========================================
# buat tabel audit_log untuk menyimpan log
...
-- perintah ini perlu dijalankan melalui command prompt
-- ==========================================
-- Backup Data:
-- mysqldump -u root -p sahabat_satwa > sahabat_satwa_backup.sql

-- Restore Data:
-- mysql -u root -p sahabat_satwa < sahabat_satwa_backup.sql
