...
require_once './../Connection/PetugasAdministrasiConnection.php';
...
if (! empty($type)) {
    if ($type == 'periksa' && ! empty($queue_id) && ! empty($vet_id)) {
        $query = "UPDATE `queue` SET 
                    `queue_status` = 'doktercheck',
                    `vet_id` = $vet_id 
    ...
    } else if ($type == 'insert' && ! empty($animal_id)
        && ! empty($clinic_id)
        && ! empty($owner_id)
        && ! empty($queue_number)
        && ! empty($queue_date)) {
